...
export function EditPropertyDialog({ 
  ...
  useEffect(() => {
    if (property) {
      setForm({
        ...
        permutaEnabled: property.permuta.enabled,
        permutaDetails: property.permuta.details || "",
      });
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const updatedData = {
      ...
      images: images.length ? images : property.images,
      city: form.city,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-strong border-glass-border rounded-2xl max-w-2xl max-h-[90vh] overflow-y-auto z-[60]">
        <DialogHeader>
        ...
        <form onSubmit={submit} className="space-y-4">
            ...
            <div className="sm:col-span-2">
              ...
              {images.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {images.map((src, i) => (
                    <div key={i} className="relative h-16 w-20 rounded-lg overflow-hidden border border-border">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button type="submit" className="bg-gradient-primary text-primary-foreground rounded-xl">Salvar alterações</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
